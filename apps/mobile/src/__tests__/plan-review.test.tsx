...
jest.mock('../lib/api', () => ({
  ...
  // apiFetch is used by RecipeSwapSheet (rendered but not interacted with here).
...
describe('Plan review (W04)', () => {
  it('shows skeleton while loading', () => {
    // Never resolves -> stays in loading state.
    mockGetWeeklyPlan.mockReturnValue(new Promise<PlanWithMealsAndRecipes>(() => {}));
    mockGetHousehold.mockReturnValue(new Promise<FamilyResponse>(() => {}));

    const { getByLabelText } = render(<PlanReviewScreen />);
    expect(getByLabelText('Ładowanie planu')).toBeTruthy();
  });

  it('blocks approval and shows a warning banner when a meal violates an allergy', async () => {
    ...
    await waitFor(() =>
      expect(getByLabelText('Ostrzeżenie o alergii')).toBeTruthy(),
    );
    // The conflict line lists the offending recipe + matched allergen and is
    // unique to the warning banner (the meal title also renders in the card).
  ...
  it('enables approval and calls approvePlan for a clean plan', async () => {
