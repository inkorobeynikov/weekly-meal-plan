...
- [x] 13c LLM rewrite/normalize `scripts/process-recipes.ts` — for each raw file not yet in DB (by `contentHash`): gpt-4o-mini structured output rewrites steps in own words (imperative Polish), normalizes ingredients (name/quantity/metric unit), infers difficulty/cost/time/servings/cuisine/tags/mealTypes/allergens/isGoodForLeftovers; Zod-validates (rule #4, allergens from the canonical enum only) before `recipeImportService.upsertImportedRecipe` (householdId NULL, source 'imported', validationStatus 'valid'). A `dishRole` classification gate keeps desserts/sides/breads/preserves out of the pool, with verdicts persisted to `data/raw-recipes/.not-meals.json` so re-runs never re-pay for them. CLI: `--dry-run`, `--force`, `--limit`, `--site`; progress + token/cost counter; `pnpm pool-stats` for monitoring. Seeded 353 obiady/kolacje (250/261 from kwestiasmaku dania-obiadowe, 91/284 from the dessert-heavy aniagotuje sitemap), 0 failures, ≈$0.44 total
- [x] 13d Pool-based plan generation behind `PLAN_FROM_POOL=1` — `recipeService.findCandidates`: **SQL-level allergen HARD-CONSTRAINT pre-filter** (`matchCanonicalAllergens` in `packages/shared` maps free-text family allergies/hardRestrictions → canonical enum; jsonb `?|` exclusion so forbidden recipes never reach the prompt), dont_repeat feedback exclusion (90 days), soft scoring (time budget, budgetMode vs costLevel, likes/preferredCuisines, jitter) → ~50 candidates. `WeeklyPlanFromPoolSchema` + `PoolReplacementSchema`: AI selects recipeId per slot on the fast model (29s vs ~60s+ ad-hoc); selections validated against the offered set incl. lunch_leftover linkage; textual post-generation guard kept as second line. `generateWeeklyPlan` + `replaceMeal` fall back to unchanged ad-hoc generation on any pool failure. Planned meals reference shared global recipes — stable identities make dishFeedback compound
- [x] 13e "Przepisy" = the FAMILY cookbook (PR-4) — deliberately NOT a browsable catalog of the imported 353-recipe pool (product + legal). Recipes surface only through plans and the family's own history. `GET /api/recipes/family` (`withAuth`, thin → `recipeService.getFamilyCookbook`): recipes the household interacted with via `planned_meals` (incl. `cooked_at`) + `dishFeedback`, with per-recipe `lastCookedAt` / `timesPlanned` / `reactions` / `isFavorite` / `isRequested`, grouped into "Ulubione" / "Ostatnio gotowane" / "Wszystkie wasze dania". Heart favorite toggle reuses the `dishFeedback` 'favorite' reaction (idempotent — one row per household+recipe, `setFavorite`); on W02 + cookbook cards. "Dodaj do następnego planu" request queue (new `recipe_requests` table, migration `0007_sleepy_rafael_vega`, `householdId`/`recipeId`/`consumedByPlanId`): `findCandidates` force-offers active requests as must-offer candidates **only if they pass the allergen HARD-CONSTRAINT SQL filter (never bypassed)** and the pool prompt marks them `[PROŚBA RODZINY]`; consumed on plan generation. Przepisy tab unhidden (removed `href: null`); cookbook screen with sections + in-family search + new-household empty state ("Twoje dania pojawią się tu po pierwszym planie"). Migrations verified clean on a fresh DB (8 migrations apply; journal/snapshot chain intact). `PLAN_FROM_POOL` default NOT flipped. Typecheck green across 9 packages; mobile Jest 15 suites / 79 tests pass
